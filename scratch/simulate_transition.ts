...
const db = getDb();
// 1. Manually set degree = 1 to simulate accepted connection
db.prepare("UPDATE targets SET degree = 1 WHERE full_name = 'Marcin Golis'").run();
// 2. Run sync to clear waiting_for_condition
const { syncAcceptedConnections } = require("../lib/linkedin/sync-accepted");
async function main() {
  await syncAcceptedConnections(db);
  const s1 = db.prepare("SELECT current_step_id, waiting_for_condition, state FROM run_profile_states WHERE run_profile_id = 'f4bf9c37-2646-4a50-bdb7-2d02b160e78d'").get();
  console.log("After sync:", s1);
  ...
  db.prepare("UPDATE targets SET degree = NULL WHERE full_name = 'Marcin Golis'").run();
