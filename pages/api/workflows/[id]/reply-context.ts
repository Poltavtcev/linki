...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "PUT") { console.log("PUT /reply-context called for", id);
    const { is_active, auto_send, sender_profile, company_product, offers_playbook, voice_rules } = req.body;
    try {
      db.prepare(`
        INSERT INTO reply_contexts (workflow_id, is_active, auto_send, sender_profile, company_product, offers_playbook, voice_rules)
        VALUES (?, ?, ?, ?, ?, ?, ?)
      ...
      `).run(id, is_active ? 1 : 0, auto_send ? 1 : 0, sender_profile, company_product, offers_playbook, voice_rules);
