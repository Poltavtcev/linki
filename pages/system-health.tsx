...
import { RiMailSendLine, RiShieldKeyholeLine, RiLinkedinBoxFill, RiTimeLine } from "react-icons/ri";
import { UserCircleIcon } from "@heroicons/react/24/outline";

export default function SystemHealth() {
  const { data, error, isLoading } = useSWR("/api/system-health", (url) => fetch(url).then(res => res.json()), { refreshInterval: 10000 });
  ...
  return (
    <div className="flex h-screen bg-base-100 font-sans text-base-content">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <TopBar title="System Health" breadcrumbs={[{ label: "Dashboard", href: "/" }]} />
        ...
        <main className="flex-1 overflow-y-auto p-6 space-y-10">
              ...
              {data?.linkedin?.map((acc: any) => (
                <div key={acc.id} className="bg-base-200 rounded-xl border border-base-300/50 p-6">
                  <div className="flex items-center gap-3 mb-6 pb-4 border-b border-base-300/40">
                    <UserCircleIcon className="w-8 h-8 text-base-content/40" />
          ...
          {/* GUARD TRIPS & LOGS */}
          <section className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-6 border-t border-base-300/50 pb-10">
            {/* Guard trips */}
            <div className="bg-base-200 rounded-xl border border-base-300/50 overflow-hidden flex flex-col">
              ...
              <div className="divide-y divide-base-300/20 flex-1 max-h-80 overflow-y-auto">
            ...
            <div className="bg-base-200 rounded-xl border border-base-300/50 overflow-hidden flex flex-col">
              ...
              <div className="divide-y divide-base-300/20 flex-1 max-h-80 overflow-y-auto font-mono">
                  ...
                  return (
                    <div key={i} className="px-5 py-2 flex items-start justify-between gap-4 hover:bg-base-300/20 transition-colors">
                      <div className="truncate">
                        <div className="text-[11px] text-base-content/80 truncate">{l.message.replace("Email sent to ", "")}</div>
